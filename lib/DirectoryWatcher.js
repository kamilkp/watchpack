...
function DirectoryWatcher(arg, options) {
	var directoryPath;
	if (typeof arg === 'string') {
		directoryPath = arg;
	} else {
		directoryPath = arg[0];
		this.isRecursive = arg[1];
		this.regex = arg[2];
		this.originalContext = arg[3];
	}
...
DirectoryWatcher.prototype.matchesRegex = function (filePath) {
	if (this.regex) {
		var relative = path.relative(this.originalContext, filePath);
		if (!this.regex.test(relative)) {
			return;
		}
	}
...
DirectoryWatcher.prototype.createNestedWatcher = function(directoryPath) {
	var nestedArg = this.regex ? [directoryPath, this.isRecursive, this.regex, this.originalContext] : directoryPath;

	this.directories[directoryPath] = watcherManager.watchDirectory(nestedArg, this.options, 1);
...
DirectoryWatcher.prototype.watch = function watch(arg, startTime) {
	var filePath;
	if (typeof arg === 'string') {
		filePath = arg;
	} else {
...
DirectoryWatcher.prototype.onFileAdded = function onFileAdded(filePath, stat) {
	...
	if(!this.matchesRegex(filePath)) return;
...
DirectoryWatcher.prototype.onDirectoryAdded = function onDirectoryAdded(directoryPath /*, stat */) {
	...
	if(!this.matchesRegex(directoryPath)) return;
